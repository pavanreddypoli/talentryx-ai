  ...
  return (
    <Card variant="light-gradient">
      <CardContent className="py-5 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          {isEditing ? (
            ...
            <div className="group flex items-center gap-2 cursor-pointer w-fit" onClick={startEdit}>
              ...
              {saveSuccess && <span className="text-xs text-emerald-600 font-normal">Saved</span>}
            </div>
          )}
        </div>
        ...
        {/* Active role — read-only badge (brand-amber for both roles; label differentiates) */}
        <div className="space-y-1.5">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Active role</p>
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-brand-amber/10 text-amber-700 border border-brand-amber/30">
            {roleLabel}
          </span>
    ...
    <Card variant="light-gradient">
      <CardContent className="py-5 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 max-w-xs">
          ...
          <Button type="submit" variant="brand-primary" size="sm"
