...
// ── Pure ranking helpers (mirrors /api/rank logic) ────────────────────────────
...
function buildKeywords(jd: string) {
  const stopwords = new Set([
    "and", "or", "the", "a", "an", "for", "to", "of", "in", "with",
...
function computeMatch(jdKeywords: string[], resume: string) {
  ...
  return { matched, missing, matchPercent, score: matchPercent / 100 };
}
...
function bucketize(keywords: string[]) {
  ...
  const buckets = { cloud:[] as string[], data:[] as string[], backend:[] as string[], frontend:[] as string[], devops:[] as string[], security:[] as string[], leadership:[] as string[], general:[] as string[] };
...
function makeInsights(name: string, matchPercent: number, matched: string[], missing: string[]) {
  ...
  for (const [label, arr] of strengthCats) {
    ...
    if (top.length > 0) { strengths.push(`${label}: ${top.join(", ")}.`); break; }
  }
  ...
  for (const [label, arr, advice] of gapCats) {
    const top = pickTop(arr, 4);
    if (top.length > 0) { gaps.push(`${label} appear missing: ${top.join(", ")}. ${advice}`); break; }
  }
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  // Ownership check via RLS — server client returns null for unowned jobs
  const supabase = await createSupabaseServerClient();
  const { data: job, error: jobErr } = await supabase
    .from("jobs")
    .select("description, jd_updated_at")
    .eq("id", jobId)
    .single();
  ...
  // Sessions whose candidates were ranked against the old JD
  const { data: staleSessions } = await supabaseAdmin
    .from("ranking_sessions")
    .select("id")
    .eq("job_id", jobId)
    .lt("created_at", job.jd_updated_at);

  const staleIds = staleSessions?.map((s) => s.id) ?? [];

  if (staleIds.length === 0) {
    return NextResponse.json({ reRanked: 0, total: 0, skipped: 0 });
  }

  const { data: staleCandidates } = await supabaseAdmin
    .from("ranking_results")
    .select("id, full_text, candidate_name")
  ...
  const jdKeywords = buildKeywords(normalizeText(job.description));
  let reRanked = 0;
  let skipped = 0;

  for (const candidate of staleCandidates) {
    if (!candidate.full_text) { skipped++; continue; }

    try {
      const resumeText = normalizeText(candidate.full_text);
      const { matched, missing, matchPercent, score } = computeMatch(jdKeywords, resumeText);
      const { strengths } = makeInsights(candidate.candidate_name, matchPercent, matched, missing);
      ...
      const { error: updateErr } = await supabaseAdmin
        ...
        .update({
          ...
          summary: strengths,
          snippet: candidate.full_text.slice(0, 400),
        })
        .eq("id", candidate.id);

      if (updateErr) {
        console.error("rerank update error for", candidate.id, updateErr);
        skipped++;
      } else {
        reRanked++;
      }
    } catch (err) {
      console.error("rerank error for candidate", candidate.id, err);
      skipped++;
    }
  }

  return NextResponse.json({ reRanked, total: staleCandidates.length, skipped });
}
