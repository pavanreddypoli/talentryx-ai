...
function FreeCard() {
  ...
  const finalPrice = applied
    ? (BOOST_PRICE * (1 - pct / 100)).toFixed(2)
    : null;
      ...
      <CardContent className="py-5 space-y-5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-baseline gap-2">
            {applied ? (
              <>
                <span className="text-sm line-through text-slate-400">${BOOST_PRICE}/mo</span>
                <span className="text-base font-bold text-slate-900">${finalPrice}/mo</span>
              </>
            ) : (
              <span className="text-sm font-semibold text-slate-700">${BOOST_PRICE}/month</span>
            )}
          </div>
          ...
          <Button variant="brand-primary" onClick={handleUpgrade} disabled={isLoading}>
            {isLoading
              ...
              : `Upgrade to Boost — ${applied ? `$${finalPrice}` : "$9"}/month`}
          </Button>
          <div>
